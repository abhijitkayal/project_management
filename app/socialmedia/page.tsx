...
export default function SocialMediaPage() {
  ...
  const [postName, setPostName] = useState("Untitled social media project");
  ...
  const socialPosts = useMemo<SocialMediaRecord[]>(() => {
    return projects.flatMap((project) => {
      const projectPosts = databasesByProject[project._id] as DatabaseRecord[] | undefined;
      return projectPosts
        ? projectPosts
            .filter((db) => db.viewType === "socialmedia")
            .map((db) => ({
              _id: db._id,
              name: db.name,
              icon: db.icon,
  ...
  const selectedPost = socialPosts.find((item) => item._id === selectedDatabaseId) ?? null;

  const handleCreateSocialMedia = async () => {
    ...
    try {
      const res = await fetch("/api/databases", {
        ...
        body: JSON.stringify({
          ...
          name: postName.trim() || "Untitled social media project",
          icon: "📱",
          viewType: "socialmedia",
          templateName: "blank",
        }),
      });
      ...
      if (!res.ok) {
        throw new Error(data?.error || "Failed to create social media project");
      }

      await fetchDatabases(targetProjectId);
      router.push(`/socialmedia?db=${data._id}`);
    } catch (error) {
      console.error("Failed to create social media project:", error);
    } finally {
      setCreating(false);
    }
  };
      ...
      <div className={`min-h-screen ${isDark ? "bg-slate-950 text-white" : "bg-slate-50 text-slate-900"}`}>
          ...
          <div className="mx-auto flex max-w-[1600px] items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3 text-white">
              ...
              <div>
                ...
                <h1 className="text-lg font-semibold leading-tight">{selectedPost.name}</h1>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="h-[calc(100vh-57px)] overflow-hidden bg-slate-100 p-4 sm:p-6">
          <div className="mx-auto h-full max-w-[1600px] overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/5">
            <SocialMediaView databaseId={selectedPost._id} />
          </div>
        </div>
      </div>
    );
    ...
    <div className={`min-h-screen ${isDark ? "bg-slate-950 text-white" : "bg-slate-50 text-slate-900"}`}>
      <div className="relative overflow-hidden border-b border-white/10 bg-linear-to-br from-slate-950 via-slate-900 to-fuchsia-950 px-6 py-10 text-white">
        <div aria-hidden="true" className="pointer-events-none absolute inset-0 opacity-30">
          ...
          <div className="absolute right-[10%] top-[4%] h-64 w-64 rounded-full bg-pink-500/25 blur-3xl" />
          <div className="absolute bottom-[10%] left-[45%] h-72 w-72 rounded-full bg-orange-400/20 blur-3xl" />
        </div>
          ...
          <div className="mt-5 grid gap-6 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
            <div>
              <h1 className="max-w-3xl text-4xl font-bold tracking-tight sm:text-5xl">
                Open a post, create a new one, and keep each campaign tied to its own database id.
              </h1>
              <p className="mt-4 max-w-2xl text-sm leading-6 text-white/70 sm:text-base">
                The sidebar now lands here. Pick an existing social media workspace or create a new project-scoped post, then all edits save against that record.
              </p>
            </div>
            ...
            <div className="rounded-3xl border border-white/10 bg-white/8 p-4 shadow-2xl shadow-black/15 backdrop-blur-xl">
              ...
              <div className="mt-4 space-y-3">
                ...
                >
                  <Globe size={16} />
                  {creating ? "Creating..." : "Create social media project"}
                </button>
              </div>
            </div>
          </div>
      ...
      <div className="mx-auto max-w-7xl px-6 py-8">
        <div className="mb-5 flex items-center justify-between gap-3">
          <div>
            <h2 className="text-lg font-semibold">Existing social media projects</h2>
            <p className={`text-sm ${isDark ? "text-slate-400" : "text-slate-500"}`}>
              Each project is backed by its own database document.
            </p>
          </div>
          <div className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold ${isDark ? "bg-white/5 text-slate-300" : "bg-slate-100 text-slate-600"}`}>
            <FolderOpen size={14} />
            {socialPosts.length} projects
          </div>
        </div>

        {socialPosts.length === 0 ? (
          <div className={`rounded-3xl border p-10 text-center ${isDark ? "border-white/10 bg-white/5" : "border-slate-200 bg-white"}`}>
            <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-linear-to-br from-fuchsia-500 to-orange-500 text-white shadow-lg shadow-fuchsia-500/20">
              <Globe size={24} />
            </div>
            <h3 className="mt-4 text-xl font-semibold">No social media projects yet</h3>
            <p className={`mt-2 text-sm ${isDark ? "text-slate-400" : "text-slate-500"}`}>
              Create the first project from the card above. It will open with its own id and persist directly to the database.
            </p>
          </div>
        ) : (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-linear-to-br from-fuchsia-500 to-orange-500 text-xl text-white shadow-lg shadow-fuchsia-500/20">
                      {item.icon || "📱"}
                    </div>
                    <div>
