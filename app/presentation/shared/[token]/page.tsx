...
export default function SharedPresentationPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const res = await fetch(`/api/presentation-shared/${token}`);
        const data = (await res.json().catch(() => ({}))) as SharedPresentationResponse;

        if (!res.ok || !data.presentation || !data.permission) {
          setError(data.error || "Failed to load shared presentation");
          setIsLoading(false);
          return;
        }
        ...
        document.cookie = `presentation_share_token=${encodeURIComponent(String(token))}; path=/; max-age=86400; samesite=lax`;
      } catch {
        setError("Failed to load shared presentation");
      }
    ...
    return () => {
      document.cookie = "presentation_share_token=; path=/; max-age=0; samesite=lax";
    };
  }, [token]);
  ...
  useEffect(() => {
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      if (target.closest(".home-button") || target.closest(".nav-button")) {
        return;
      }
  ...
  if (isLoading) {
    return (
      <div className={`min-h-screen ${isDark ? "bg-gray-900 text-white" : "bg-gray-50 text-gray-900"}`}>
        <SpinnerFullscreen text="Loading shared presentation..." />
      </div>
    );
  }

  if (error || !presentation?._id) {
    return (
    ...
    <div className={`min-h-screen ${isDark ? "bg-slate-950 text-white" : "bg-slate-50 text-slate-900"}`}>
      <div className="border-b border-white/10 bg-linear-to-r from-violet-600 via-indigo-600 to-fuchsia-600 px-4 py-3 shadow-lg shadow-black/10">
        <div className="mx-auto flex max-w-[1600px] items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3 text-white">
            ...
            <div>
              <div className="text-[11px] uppercase tracking-[0.24em] text-white/70">Shared deck</div>
              <h1 className="text-lg font-semibold leading-tight">{presentation.name}</h1>
            </div>
          </div>
        </div>
      </div>
      ...
      <div
        className={`border-b px-6 py-3 ${
          permission === "edit" ? "border-green-500/30 bg-green-500/10" : "border-blue-500/30 bg-blue-500/10"
        }`}
      >
          ...
          {permission === "edit" ? (
            <>
              ...
              <span className={isDark ? "text-green-300" : "text-green-700"}>
                You have edit access to this presentation.
              </span>
            </>
          ) : (
            <>
              ...
              <span className={isDark ? "text-blue-300" : "text-blue-700"}>
                You have view-only access to this presentation.
              </span>
            </>
          )}
        </div>
      ...
      <div className="h-[calc(100vh-104px)] overflow-hidden bg-slate-100 p-4 sm:p-6">
        <div className="mx-auto h-full max-w-[1600px] overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/5">
          <PresentationView databaseId={presentation._id} />
        </div>
      </div>
    </div>
  );
