...
export default function WhiteboardPage() {
  ...
  const whiteboards = useMemo<WhiteboardRecord[]>(() => {
    return projects.flatMap((project) => {
      const projectBoards = databasesByProject[project._id] as DatabaseRecord[] | undefined;
      return projectBoards
        ? projectBoards
        .filter((db) => db.viewType === "whiteboard")
        .map((db) => ({
          _id: db._id,
          name: db.name,
          icon: db.icon,
  ...
  const selectedBoard = whiteboards.find((board) => board._id === selectedDatabaseId) ?? null;

  useEffect(() => {
    if (!selectedDatabaseId && selectedBoard) {
      router.replace(`/whiteboard?db=${selectedBoard._id}`);
    }
  }, [router, selectedDatabaseId, selectedBoard]);
    ...
    try {
      const res = await fetch("/api/databases", {
        ...
        body: JSON.stringify({
          ...
          name: boardName.trim() || "Untitled whiteboard",
          icon: "✏️",
          viewType: "whiteboard",
          templateName: "blank",
        }),
      ...
      router.push(`/whiteboard?db=${data._id}`);
    } catch (error) {
      ...
      <div className={`min-h-screen ${isDark ? "bg-slate-950 text-white" : "bg-slate-50 text-slate-900"}`}>
        <div className="border-b border-white/10 bg-linear-to-r from-teal-600 via-cyan-600 to-rose-600 px-4 py-3 shadow-lg shadow-black/10">
          <div className="mx-auto flex max-w-[1600px] items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3 text-white">
              ...
              <div>
                <div className="text-[11px] uppercase tracking-[0.24em] text-white/70">Current board</div>
                <h1 className="text-lg font-semibold leading-tight">{selectedBoard.name}</h1>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="h-[calc(100vh-57px)] overflow-hidden bg-slate-100 p-4 sm:p-6">
          <div className="mx-auto h-full max-w-[1600px] overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/5">
            <Whiteboard databaseId={selectedBoard._id} />
          </div>
        </div>
      </div>
    );
    ...
    <div className={`min-h-screen ${isDark ? "bg-slate-950 text-white" : "bg-slate-50 text-slate-900"}`}>
      <div className="relative overflow-hidden border-b border-white/10 bg-linear-to-br from-slate-950 via-slate-900 to-teal-950 px-6 py-10 text-white">
        <div aria-hidden="true" className="pointer-events-none absolute inset-0 opacity-30">
          ...
          <div className="absolute right-[10%] top-[4%] h-64 w-64 rounded-full bg-rose-500/25 blur-3xl" />
          <div className="absolute bottom-[10%] left-[45%] h-72 w-72 rounded-full bg-emerald-400/20 blur-3xl" />
        </div>
          ...
          <div className="mt-5 grid gap-6 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
            <div>
              <h1 className="max-w-3xl text-4xl font-bold tracking-tight sm:text-5xl">
                Open a board, create a new one, and keep each canvas tied to its own database id.
              </h1>
              <p className="mt-4 max-w-2xl text-sm leading-6 text-white/70 sm:text-base">
                The sidebar now lands here. Pick an existing whiteboard or create a new project-scoped board, then all canvas changes save against that database record.
              </p>
            </div>
      ...
      <div className={`mx-auto max-w-7xl px-6 py-8 ${isDark?"bg-transparent":"bg-white"}`}>
        <div className="mb-5 flex items-center justify-between gap-3">
          <div>
            ...
            <p className={`text-sm ${isDark ? "text-slate-400" : "text-slate-500"}`}>
              Each board is backed by its own database document.
            </p>
          </div>
        ...
        {whiteboards.length === 0 ? (
          <div className={`rounded-3xl border p-10 text-center ${isDark ? "border-white/10 bg-white/5" : "border-slate-200 bg-white"}`}>
            <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-linear-to-br from-teal-500 to-cyan-500 text-white shadow-lg shadow-teal-500/20">
              <PenTool size={24} />
            </div>
            <h3 className="mt-4 text-xl font-semibold">No whiteboards yet</h3>
            <p className={`mt-2 text-sm ${isDark ? "text-slate-400" : "text-slate-500"}`}>
              Create the first board from the card above. It will open with its own id and persist directly to the database.
            </p>
          </div>
        ) : (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-linear-to-br from-teal-500 to-cyan-500 text-xl text-white shadow-lg shadow-teal-500/20">
                      {board.icon || "✏️"}
                    </div>
                    <div>
                      <h3 className="font-semibold leading-tight text-slate-950 dark:text-white">{board.name}</h3>
                      <p className={`text-sm ${isDark ? "text-slate-400" : "text-slate-500"}`}>
                        {board.projectEmoji || "📁"} {board.projectName || "Project"}
                      </p>
                    </div>
                  </div>
                ...
                <div className="mt-4 text-xs text-slate-500 dark:text-slate-400">
                  Updated {board.updatedAt ? new Date(board.updatedAt).toLocaleString() : "recently"}
                </div>
              </button>
            ))}
          </div>
