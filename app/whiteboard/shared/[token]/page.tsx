    ...
    const load = async () => {
      ...
      try {
        ...
        if (!res.ok || !data.resource || !data.permission) {
          setError(data.error || "Failed to load shared whiteboard");
          setIsLoading(false);
          return;
        }

        if (data.resource.viewType !== "whiteboard") {
          setError("This share link is not for a whiteboard.");
          setIsLoading(false);
          return;
        }
  ...
  if (isLoading) {
    return (
      <div className={`min-h-screen ${isDark ? "bg-gray-900 text-white" : "bg-gray-50 text-gray-900"}`}>
        <SpinnerFullscreen text="Loading shared whiteboard..." />
      </div>
    );
  }
    ...
    <div className={`min-h-screen ${isDark ? "bg-slate-950 text-white" : "bg-slate-50 text-slate-900"}`}>
      <div className="border-b border-white/10 bg-linear-to-r from-teal-600 via-cyan-600 to-rose-600 px-4 py-3 shadow-lg shadow-black/10">
        <div className="mx-auto flex max-w-[1600px] items-center justify-between gap-3">
          <button type="button" onClick={() => router.push("/whiteboard")} className="inline-flex items-center gap-2 rounded-xl bg-white/10 px-3 py-2 text-sm font-medium text-white backdrop-blur hover:bg-white/20 transition">
            <ArrowLeft size={16} />
            Whiteboard library
          </button>
          <div className="flex items-center gap-3 text-white">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-white/15 text-lg backdrop-blur">{resource.icon || "✏️"}</div>
            <div>
              <div className="text-[11px] uppercase tracking-[0.24em] text-white/70">Shared board</div>
      ...
      <div className={`border-b px-6 py-3 ${permission === "edit" ? "border-green-500/30 bg-green-500/10" : "border-blue-500/30 bg-blue-500/10"}`}>
        <div className="mx-auto flex max-w-[1600px] items-center gap-2 text-sm">
          {permission === "edit" ? (
            <><Edit className="h-4 w-4 text-green-400" /><span className={isDark ? "text-green-300" : "text-green-700"}>You have edit access to this whiteboard.</span></>
          ) : (
            <><Eye className="h-4 w-4 text-blue-400" /><span className={isDark ? "text-blue-300" : "text-blue-700"}>You have view-only access to this whiteboard.</span></>
          )}
        </div>
      </div>
      ...
      <div className="h-[calc(100vh-104px)] overflow-hidden bg-slate-100 p-4 sm:p-6">
        <div className="mx-auto h-full max-w-[1600px] overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/5">
          <Whiteboard databaseId={resource._id} />
        </div>
      </div>
    </div>
  );
